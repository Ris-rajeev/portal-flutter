...
class CreateWireframeScreen extends StatefulWidget {
  final int moduleId; // Add moduleId parameter

  CreateWireframeScreen({required this.moduleId, Key? key}) : super(key: key);

  @override
  _CreateWireframeScreenState createState() => _CreateWireframeScreenState();
}

class _CreateWireframeScreenState extends State<CreateWireframeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Create Wireframe')),
            ...
            child: Column(
              children: [
                TextFormField(
                  ...
                  onSaved: (value) => formData['uiName'] = value,
                ),
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        ...
                        formData['child_form'] = ischildform;
                        formData['moduleId'] = widget.moduleId;

                        final token = await TokenManager.getToken();
                        try {
                          print(formData);

                          await wireframeService.createWireframe(
                              token!, formData);
                          ...
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                ...
                                content: Text('Failed to create wireframe: $e'),
                                actions: [
                                  TextButton(
                                    child: const Text('OK'),
                                    onPressed: () {
